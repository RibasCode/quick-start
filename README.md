
<!--

PROJECT: quick-start

   - All basics already set up
   - More info ./quick-start/README.md

-->

<!--

PROJECT: quick-start-dev

   - Dev propouses (Learn and train, best practices, implementation, basic elements, test depencencyes)
   - More info ./quick-start-dev/README.md

   - Un cop acabem de manipular el projecte quick-start-dev:
      - Al quick-start esborrem la carpeta css, js i scss.
      - Copiem la carpeta js i scss del quick-start-dev i les enganxem al quick-start.
      - Dins del quick-start, esborrem l'arxiu js/components/toast.js
      - Dins del quick-start, a scss/style.scss esborrem el @inport del toastify.css
      - Esborrem el scss/base/_others.scss i el seu @import dins del index.scss
      - Compilem el quick-start
      - Comprobem que en el git status no apareix res de quick-start-dev/... per comprobar que no s'ha esborrat res per error.

-->
